# Nord Liberta — Especificação do Produto · v3 (CONVERGIDA — 5/5 no Gauntlet)

> Estado: **CONVERGIDA**. Na Rodada 3 os 5 especialistas declararam **SATISFEITO** sobre a v2.
> A v3 consolida os refinamentos **não-bloqueantes** (IMPORTANTE/MENOR) que o painel satisfeito
> ainda pediu — nenhum reabre veredito. Governada por `../00-contexto-v5.md` e pelos princípios
> invioláveis (§9). Conteúdo financeiro oficial pendente está no **Anexo A (Lacunas)** — nada é
> inventado. Exemplos `ILUSTRATIVO` usam persona fictícia (Ricardo) e **não** são metodologia oficial.
>
> **Selo:** Cliente ✅ · Psicólogo ✅ · Redator ✅ · CFP ✅ · Engenheiro ✅ (Rodada 3).
...
- **Rollback:** consultor pode reabrir `coletaEmAndamento` a partir de `devolutivaMontada`/`cicloMensal`
  (recomputa capítulos `naoIniciado`, congela os `visto` — §3.2).
- **Portas de `suspenso` (refino Eng):** entra em `suspenso` a partir de **qualquer** fase pós-exame,
  disparado por **cliente** (pausa voluntária) ou **admin/consultor** (inadimplência/pausa contratual);
  registrado em `AuditLog`. Retomada volta à **fase de origem** (guardada em `faseAntesSuspensao`), com
  o ponteiro de retomada da §13. `renovacao` só é alcançada da `semestral` do 6º ciclo (Fase 6 = Anexo A).
...
ValorColetado       { id, clienteId, chave, valor, proveniencia: declarado|estimado|validado,
                      ...
                      // versao: inteiro incrementado a cada UPDATE do consultor (âncora de dadoMudou, §7)
...
AuditLog            { id, entidade, entidadeId, campo, valorAntes, valorDepois, ator, acao, em, hashAnterior? }
                      // append-only; hashAnterior encadeia à prova de adulteração EM PRODUÇÃO (no mock é convenção)
```
...
> estrutura mas não produz Projeto/decisão sem ela. **Sinalização explícita (refino CFP):** enquanto
> inerte, a vertical aparece ao consultor (no radar e na montagem da devolutiva) com o selo
> **"Ativos: aguardando metodologia"** — nunca some silenciosamente, para não parecer que a carteira
> do cliente foi esquecida.
...
2. **Os seus números** — 2–3 cards via `CardTemplate.formulaRef`, cada um com "como chegamos nesse
   número" e proveniência herdada pela regra §3.3. **Peak afetivo do capítulo** = o card de impacto
   humano ("de 11 meses para 8 anos de autonomia") — é o *reveal* que gera afeto (refino Psi: o pico
   é aqui, não na lista de tarefas).
...
4. **As tarefas nascem** — "N tarefas entraram no seu plano de ação" + frame de agência ("já
   organizamos isso para você"). É o **end de descarga** logo após o peak (peak-end, §9.1).
   *Degradação do cálculo (refino Eng):* se um `CardTemplate.formulaRef` não resolve (fórmula pendente
   no Anexo A) ou uma entrada some no momento do publish, o card produz **`indeterminado`**,
   `numerosSnapshot.valor = indeterminado`, e o capítulo degrada ao consultor — mesma família de
   `campo ausente` (§6.4) e `Ativos inerte` (§5). Nunca grava um número inventado no snapshot.
...
4. *2 tarefas nascem:* "Solicitar 3 cotações" (mês 1, *quando?* "no dia do salário") · "Escolher e
   ...
   Copy da tela: "já organizamos isso **para** você" (registro premium na plataforma; "pra" só no WhatsApp).
...
**Revalidação de suitability por prazo (refino CFP):** além do auto-diff (que pega mudança de valor),
a pauta força **re-perguntar o perfil de suitability quando o prazo regulatório expira**, mesmo sem
mudança declarada — suitability vence por tempo. Prazo exato = ⚠ LACUNA (compliance, Anexo A).
...
- **Tarefa:** o lembrete é agendado pelo `Tarefa.quando` do cliente (ex.: "no dia do salário"),
  **não** por data genérica — fecha a implementation intention (Gollwitzer) e ancora o Prompt no
  ...
  - **Fallback quando `Tarefa.quando` é null (refino Psi):** `quando` é opcional e muitos não o
    declaram. Sem ele, **jamais** usar data de calendário genérica (vira *cold trigger*): ancorar por
    padrão ao **dia do salário/aporte já conhecido da coleta** (Gestão Financeira tem o campo), e usar
    o primeiro lembrete para **capturar o `quando`** ("quer que eu te lembre sempre no dia do salário?")
    — transformando a lacuna em oportunidade de firmar a intenção.
...
- **Cadência e dignidade do canal (bloqueante importante Psi):** teto de **N mensagens/mês** (Anexo
  A — começar baixo, "menos é mais"; só subir com evidência), regra de **não empilhar** (aporte +
  tarefa + comitê no mesmo dia viram **um**, e o não-empilhar tem precedência sobre o teto N),
  **opt-out por tipo**. Para 50+ premium, dignidade do canal = retenção.
...
- **Win-back respeita opt-out e teto (refino Psi):** o silencioso é o mais propenso a ter dado opt-out
  e a sentir *reactance* — quem mais precisa de dignidade de canal não pode virar o mais bombardeado.
  As mensagens de win-back **contam dentro do teto N/mês** e **respeitam o opt-out por tipo**; o único
  canal-âncora preservado mesmo sob opt-out é o **handoff humano do ≥3º ciclo** (é pessoa, não automação).
...
**Todos os objetivos na régua (refino Cliente):** a linha do tempo instancia **cada** objetivo como
marco na trajetória — não só a aposentadoria. Objetivos curtos (proteção da família, reserva de 6
meses, sucessão) aparecem como **marcos com data prevista de conclusão**, não apenas como tarefas no
plano; o cliente vê *quando* cada um "fica de pé".

`ILUSTRATIVO` — **linha do tempo do Ricardo (todos os objetivos):**
- **Proteção da família** — conclui ~mês 2 (ao contratar a apólice). Marco curto.
- **Reserva de emergência (6 meses)** — conclui ~mês 8. Marco curto.
- **Tranquilidade sucessória** — conclui ~mês 3 (testamento). Marco curto.
- **Venda da ótica com tranquilidade** — evento **~2036 (aos 64)** (refino Cliente: é o maior evento
  financeiro da vida dele e não podia faltar). Modela a **liquidez futura** e como ela **conversa com
  a aposentadoria** — a tela mostra o evento de venda no meio do caminho e o efeito da liquidez na data de 2041.
- **Aposentadoria** — anoAlvo **2041** (aos 69), aporte combinado R$10.000/mês.
...
- **Fio explícito aporte → data:** o número informado no ciclo (§9.1.3) **move a barra** desta tela;
  o "adicionou 4 meses de autonomia" (§9.1.5) é a versão sentida do mesmo cálculo. Método = Anexo A.
- **Conta do aporte combinado (refino Cliente):** deixar explícito na tela que o **aporte combinado
  (R$10.000/mês) é o valor destinado aos objetivos de acumulação** (ex.: aposentadoria); despesas de
  tarefa que são **custo recorrente de proteção** (seguro R$190/mês) ou **realocação** (PGBL, reserva)
  são sinalizadas como **fora** ou **dentro** do combinado, item a item — o cliente nunca soma de
  cabeça nem confunde "guardar" com "gastar para proteger". Regra de exibição definida; valores = Anexo A.
- **Consolidado "estou no rumo no todo?" (refino Cliente):** a visão agregada mostra **como um objetivo
  atrasado e um adiantado se compensam** — ex.: proteção concluída adiantada + aposentadoria em 2043 →
  o consolidado explica a soma ("3 de 5 objetivos no rumo ou concluídos; 1 pede atenção"), nunca um
  número único opaco.
...
- 0–20 (coral): "Sua saúde financeira está no começo da construção — há bases a montar por aqui."
...
**SLA-teto por item (refino CFP):** cada item tem um **tempo máximo de espera** — mesmo baixa
severidade não "aguarda indefinidamente". Ao atingir o teto de espera, o item **sobe de prioridade e
escala ao admin**, porque "silêncio nunca é zero" vale também para o lado do consultor: baixa
severidade acumulada não pode virar cliente abandonado sob a métrica verde. Valores de SLA = Anexo A.
...
- **Parâmetros operacionais:** teto de mensagens/mês (§9.2), `clientesPorConsultor`/`tetoFilaDiaria`,
  SLAs e **SLA-teto de espera por item** (§15), regra de pró-rata sem cascata (§10.2).
- **Mapa `perfil de suitability` → variante de broadcast do comitê** (§9.1.2) — tabela de correspondência (consultoria/compliance).
- **Prazo regulatório de revalidação de suitability** (§8) (compliance).
- **Base legal/retenção LGPD** (jurídico).
- **A monitorar em piloto (não bloqueia):** taxa de `indeterminado` no motor de elegibilidade (§6.4) —
  eixo que decide o ROI de tempo da fase 3; medir com a 2ª consultora antes de escalar para 150.
