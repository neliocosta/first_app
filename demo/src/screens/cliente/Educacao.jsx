...
export default function Educacao({ estado, acoes }) {
    ...
    return (
      <div className="pt-4">
        <button onClick={() => { setQuiz(false); setResp(null); }}
          ...
          <Icone nome="arrow-left" size={14} /> voltar aos conceitos
        </button>
        <Card>
          ...
          <div className="space-y-2.5">
            {QUIZ_EXEMPLO.opcoes.map((o, i) => (
              <button key={i} onClick={() => { setResp(i); if (o.correta) acoes.dominarConceito(QUIZ_EXEMPLO.conceitoId); }}
                disabled={resp !== null}
                className={`w-full text-left px-4 py-3.5 rounded-btn border font-body text-sm min-h-[44px] transition-colors
                  ${resp === null ? 'border-ink-line hover:border-orange-500 text-navy-900'
                    ...
                    : resp === i ? 'border-ink-line bg-cream-100 text-navy-900' : 'border-ink-line text-ink-body opacity-60'}`}>
                {o.texto}
              </button>
            ))}
          </div>
    ...
    <div className="pt-4 space-y-5">
      ...
      <div className="rounded-module bg-white shadow-float p-6">
        <div className="flex items-end justify-between mb-4">
          ...
          <span className="font-ui text-xs text-ink-body text-right max-w-[46%]">6 você já sabia antes de começar</span>
        </div>
        <div className="h-2 rounded-full bg-cream-100 overflow-hidden">
          <div className="h-full bg-orange-700 rounded-full" style={{ width: `${dominados}%` }} />
        </div>
      </div>
          ...
          return (
            <div key={c.id}
              ...
              <span className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0
                ${dominado ? 'bg-[#1F7A45]' : 'bg-cream-100 border border-ink-line'}`}>
              ...
              {jogavel && !dominado && <Badge status="medium">responder</Badge>}
            </div>
          );
        })}
        <p className="font-ui text-xs text-white/30 text-center pt-3">
          amostra de 15 dos cerca de 100 conceitos
        </p>
      </div>
    </div>
  );
